...
{
    BOOL isGyro = [self nextGyro];
    if (isGyro) {
        [_glPanoramavc startDeviceMotion];
    }else{
        [_glPanoramavc stopDeviceMotion];
    }
    _isGyroAction = isGyro;
    [sender setImage:[UIImage imageNamed:_gyroIcons[@(isGyro)]] forState:UIControlStateNormal];
}


-(void)modeDidTouch:(UIButton *)sender
{
    ZLCPanoPerspectMode nextMode = [self nextMode];
    [_glPanoramavc setPerspectiveMode:nextMode];
    _perspectMode = nextMode;
    [sender setImage:[UIImage imageNamed:_modeIcons[@(nextMode)]] forState:UIControlStateNormal];
}

-(ZLCPanoPerspectMode)nextMode
{
    NSArray *modeKeys   = _modeIcons.allKeys;
    ZLCPanoPerspectMode mode = _perspectMode;
    if (_perspectMode == (ZLCPanoPerspectMode)[modeKeys.lastObject integerValue]) {  //选到最后一个
        mode  = (ZLCPanoPerspectMode)[modeKeys.firstObject integerValue];//回到第一个
    }else
    { //非最后一个
        int nextIndex = 0;
        for (NSNumber *num in modeKeys) {
            nextIndex++;
            if (_perspectMode == [num integerValue]) {
                break;
            }
        }
        mode = (ZLCPanoPerspectMode)[modeKeys[nextIndex] integerValue];
    }
    return mode;
}
...
-(BOOL)nextGyro
{
    return _isGyroAction?false:true;
}
