...
//    NSLog(@"curPixelRef tag2 = %p",pixelBuffer);
...
//动画过程
    ...
    if (self.srcType == ZLCPanoSRCTypeStream) {
        if (_isUsingMotion)
        {
            ...
            {
                [self updateAtPortaitUsingMotion];
            }
            if (self.nowOrientation == INITOrientation_LandscapeRight)
            {
                [self updateAtLandscapeUsingMotion];
            }
        }else
        {
#pragma mark 横竖屏都可以用
            [self updateStreamFingerGetsture];
        }
...
-(void)updateAtLandscapeUsingMotion
{
...
//    NSLog(@"changing______ w = %f  wx = %f  wy = %f wz = %f ",w,wx,wy,wz);
...
-(void)updateAtPortaitUsingMotion
{
    float aspect =   (self.view.bounds.size.width / self.view.bounds.size.height);
    GLKMatrix4 projectionMatrix = GLKMatrix4MakePerspective(GLKMathDegreesToRadians(n_overtrue), aspect, 0.1f, 10000.0f);
    projectionMatrix = GLKMatrix4Rotate(projectionMatrix, ES_PI, self.camRotX, 0.0f, 0.0f);
...
//    NSLog(@"updatePortaitStreamUsingMotion_%f_%f",self.fingerRotationX,self.fingerRotationY);
    modelViewMatrix = GLKMatrix4RotateX(modelViewMatrix, M_PI_2);
    self.modelViewProjectionMatrix = GLKMatrix4Multiply(projectionMatrix, modelViewMatrix);
    glUniformMatrix4fv(uniforms[UNIFORM_MODELVIEWPROJECTION_MATRIX], 1, GL_FALSE, self.modelViewProjectionMatrix.m);
}
...
-(void)updateStreamFingerGetsture
{
    float aspect =   (self.view.bounds.size.width / self.view.bounds.size.height);
    GLKMatrix4 projectionMatrix = GLKMatrix4MakePerspective(GLKMathDegreesToRadians(n_overtrue), aspect, 0.1f, 10000.0f);
    projectionMatrix = GLKMatrix4Rotate(projectionMatrix, ES_PI, self.camRotX, 0.0f, 0.0f);
...
//    NSLog(@"🍊🍊🍊🍊🍊🍊🍊🍊offset = %f self.overture = %f",offset,self.overture);
    ...
    self.fingerRotationZ += 0;
//    NSLog(@"🍊🍊🍊🍊 self.fingerRotationX = %f  Y = %f",self.fingerRotationX,self.fingerRotationY);
